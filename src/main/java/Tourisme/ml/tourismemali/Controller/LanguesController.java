...
public class LanguesController {
    @Autowired
    private LanguesService service;

    // methode pour la création d'une Langue
    @ApiOperation(value = "Creation d'une langue.")
    @PostMapping("/creer")
    public ResponseEntity<Object> creerLangue(@RequestBody Langues Langue) {

        Langues verif_Langue = service.findByLibele(Langue.getLibele());
        if (verif_Langue == null) {
            Langues EnregistreLangue = service.saveLangue(Langue);
    ...
    public ResponseEntity<Object> ModifierRegion(@RequestBody Langues langue,
                                                 @PathVariable(value = "CodeRegion") String code) {

        Langues verif_Langue = service.getLangueByCode(code);
        if (verif_Langue != null) {
            Langues EnregistreLangue = service.updateLangue(langue);
